 ...
 */
typedef vector<bitset<200>> vbit;
struct Maxclique {
	...
	void init(vv& r) {
		...
		int mxD = r[0].d;
		forn(i, 0, sz(r)) r[i].d = min(i, mxD) + 1;
	}
	void expand(vv& R, int lev = 1) {
		...
		while (sz(R)) {
			...
			if (sz(T)) {
				...
				if (j > 0) T[j - 1].d = 0;
				forn(k, mnk, mxk + 1) for (int i : C[k])
					T[j].i = i, T[j++].d = k;
				expand(T, lev + 1);
			}
	...
	Maxclique(vbit conn) : e(conn), C(sz(e) + 1), S(sz(C)), old(S) {
		forn(i, 0, sz(e)) V.push_back({ i });
	}
};
