...
ll MinCostMatching(const vector<vd>& cost, vi& L, vi& R) {
	...
	forn(i,0,n) {
		u[i] = cost[i][0];
		forn(j,1,n) u[i] = min(u[i], cost[i][j]);
	}
	forn(j,0,n) {
		v[j] = cost[0][j] - u[0];
		forn(i,1,n) v[j] = min(v[j], cost[i][j] - u[i]);
	}
...
void testMatching() {
	forn(it,0,100000) {
		...
		forn(i,0,N) forn(j,0,M) co[i][j] = (rand() % 10) + 2;
		vi L, R;
		ll v = MinCostMatching(co, L, R);
		int S = N+M, T = N+M+1;
		MCMF mcmf(N+M+2);
		forn(i,0,N) mcmf.addEdge(S, i, 1, 0);
		forn(i,0,M) mcmf.addEdge(N+i, T, 1, 0);
		forn(i,0,N) forn(j,0,M) mcmf.addEdge(i, N+j, 1, co[i][j] - 2);
		mcmf.setpi(S);
		auto pa = mcmf.maxflow(S, T);
...
void testNeg() {
	...
	forn(it,0,1000000) {
		...
		MCMF mcmf(N);
		MCMF2 mcmf2(N);
		forn(i,0,N) forn(j,0,N) ed[i][j] = 0;
		forn(eid,0,M) {
