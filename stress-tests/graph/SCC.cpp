...
template<class G> vi scc(G &g) {
	...
	comp.assign(n, -1);
	forn(i,0,n) if (comp[i] == -1) dfs(i, g);
	return comp;
}
}
...
int main() {
	...
	for (int N = 0; N <= 4; N++) {
		...
		forn(bits,0,(1 << (N*N))) {
			...
			forn(i,0,N) assert(comp[i] >= 0 && comp[i] < ncomps);
			forn(i,0,N) for(auto &j: adj[i]) assert(comp[j] <= comp[i]);
			forn(i,0,N) {
				seen.assign(N, 0); seen[i] = 1;
				forn(it,0,N) {
					forn(j,0,N) if (seen[j]) for(auto &k: adj[j]) seen[k] = 1;
				}
				forn(j,0,N) {
