...
vi coverHK(vector<vi>& g, int n, int m) {
	...
	vi q, cover;
	forn(i,0,n) if (lfound[i]) q.push_back(i);
	while (!q.empty()) {
	...
	forn(i,0,n) if (!lfound[i]) cover.push_back(i);
	forn(i,0,m) if (seen[i]) cover.push_back(n+i);
	assert(sz(cover) == res);
	return cover;
}
...
int main() {
	forn(it,0,300000) {
		...
		vector<vi> gr(N);
		vi left(N), right(M);
		forn(i,0,N) forn(j,0,M) if (rand() < prop) {
			gr[i].push_back(j);
		}
		auto verify = [&](vi& cover) {
			...
			forn(i,0,N) if (!left[i]) for(auto &j:gr[i]) {
				assert(right[j]);
				/* if (!right[j]) {
					cout << N << ' ' << M << endl;
					forn(i,0,N) for(auto &j: gr[i]) cout << i << " - " << j << endl;
