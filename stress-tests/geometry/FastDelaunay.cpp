...
int main1() {
	...
	forn(it,0,3000000) {{
		...
		if (false) forn(i,0,N) forn(j,0,i) forn(k,0,j) {
			// colinear
			if (ps[i].cross(ps[j], ps[k]) == 0) {  goto fail; }
		}
		if (false) forn(i,0,N) forn(j,0,i) forn(k,0,j) forn(l,0,k) {
			// concyclic
		...
		if (N >= 3) {
			forn(i,2,N) if ((ps[i] - ps[0]).cross(ps[1] - ps[0])) allColinear = false;
		}
