...
int solveLinear(vector<bs>& A, vi& b, bs& x, int m) {
	...
	forn(i,0,n) {
		...
		swap(b[i], b[br]);
		swap(col[i], col[bc]);
		forn(j,0,n) if (A[j][i] != A[j][bc]) {
			A[j].flip(i); A[j].flip(bc);
		}
		forn(j,i+1,n) if (A[j][i]) {
			b[j] ^= b[i];
			A[j] ^= A[i];
		}
		rank++;
...
int main() {
	int ct = 0;
	forn(n,0,nmax+1) forn(m,0,mmax+1) {
		int nm = n*m;
