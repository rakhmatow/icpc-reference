...
void testTr(int n, int q) {
	...
	vi par(n), val(n);
	forn(i,1,n) par[i] = rand() % i;
	forn(i,0,n) val[i] = rand() % 1000;
	vector<vi> ed(n);
	forn(i,1,n) ed[par[i]].push_back(i), ed[i].push_back(par[i]);
	MoTree::vals = val;
	MoTree::sum = 0;
	MoTree::path.clear();
...
int main() {
	srand(2);
	forn(it,0,10) forn(n,1,15) forn(q,0,n*n) {
		testTr(n, q);
	}
